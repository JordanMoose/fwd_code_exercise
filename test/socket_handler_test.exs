defmodule FwdCodeExercise.SocketHandlerTest do
  ...
  describe "init/1" do
    ...
    test "failure path: fails to subscribe to the 'wildfires' topic and logs and returns the error from PubSub" do
      with_mock(PubSub, subscribe: fn _, _ -> {:error, :failure_reason} end) do
  ...
  describe "handle_info/2" do
    test "happy path: adds 'is_wildfire_update' => true to the GeoJSON data and pushes the stringified JSON data to the client" do
      ...
      assert {:push, {:text, json}, ^state} = SocketHandler.handle_info({:wildfire_updates, geojson}, state)

      assert json =~ ~r/"is_wildfire_update":\s*true/
      assert json =~ ~r/"IncidentName":\s*"Test wildfire"/
    end

    test "failure path: logs an error and returns the unchanged state if encoding the GeoJSON data fails" do
      geojson = %{"invalid" => "data"}
      state = %{}

      with_mock(Jason, encode: fn _, _ -> {:error, :encoding_error} end) do
        assert {{:ok, ^state}, log} = with_log(fn -> SocketHandler.handle_info({:wildfire_updates, geojson}, state) end)
    ...
    test "happy path: ignores a :wildfire_updates message if the geojson is not a map" do
      state = %{}
      assert {{:ok, ^state}, ""} = with_log(fn -> SocketHandler.handle_info({:wildfire_updates, "not_a_geojson"}, state) end)
    end

    test "happy path: ignores other messages and returns the unchanged state" do
      state = %{}
      assert {{:ok, ^state}, ""} = with_log(fn -> SocketHandler.handle_info("some_other_message", state) end)
    end
  end
  ...
  describe "handle_in/2" do
    test "happy path: logs the received message and returns the unchanged state" do
      state = %{}
      assert {{:ok, ^state}, log} = with_log(fn -> SocketHandler.handle_in({"some_message", [opcode: :text]}, state) end)
